...
- **🗂️ 实例管理**：创建 / 复制 / 删除实例；每个实例分配独立回环端口（默认 3080），旧配置自动迁移、端口冲突自动规避
- **⚙️ 运行时管理**：通过内置 Node / pnpm 按需安装指定版本的 dsh（默认 0.1.0-rc.6），版本彼此隔离，无全局 npm 安装
...
- **🛡️ 安全进程验证**：以 PID + 端口 + 实例工作区所有权三重校验，绝不误杀无关进程；仅占用端口但无法验证所有权的监听者报告为 port occupied
- **🖼️ 主题与背景**：自定义 hero 背景图，随实例切换显示
